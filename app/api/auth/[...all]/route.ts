...
const aj = arcjet({
  key: serverEnv.ARCJET_KEY, // Get your site key from https://app.arcjet.com
...
const botOptions = {
  mode: "LIVE",
  allow: ["STRIPE_WEBHOOK"], // prevents bots from submitting the form
...
async function protect(req: NextRequest): Promise<ArcjetDecision> {
  ...
  if (req.nextUrl.pathname.startsWith("/auth/get-started")) {
  ...
  // For all other auth requests (including social auth)
  ...
  return aj
    ...
    .withRule(
      slidingWindow({
        mode: "LIVE",
        interval: "15m", // 15 minute window
        max: 10, // Allow up to 10 auth attempts per 15 minutes
      })
    )
    .protect(req, { userId });
}
